...
class LR1:
    ...
    def _clear_usb_buffers(self) -> None:
        """Clear any stale data from USB input buffer"""
        try:
            cleared_count = 0
            while True:
                self.device.read(EP_IN, PACKET_SIZE_BYTES, timeout=50)
                cleared_count += 1
                if cleared_count > 10:  # Safety limit
                    break
